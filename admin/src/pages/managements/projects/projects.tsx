...
const StyledTableCell = styled(TableCell)(({ theme }) => ({
  ...
  const TextHover = {
    cursor: "pointer", ":hover": {color: Color.DarkBlue, textDecoration: 'true', fontWeight: "Bold" },
  }
...
const ListOfProjects = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await axios.get<IProject[]>(`${API_PROJECTS}/get-all-projects`);
  ...
  if (error) return <p>Error: {error}</p>;
    ...
    return (
        <Box width={"100%"} height={"100%"} display={"flex"} flexDirection={"column"} gap={"40px"}>
        ...
        <TableBody>
          {projects.map((project) => (
            ...
            >
              ...
              <StyledTableCell align="right" sx={TextHover}>{project.student_name}</StyledTableCell>
              <StyledTableCell align="right" sx={TextHover}>{project.teacher_name}</StyledTableCell>
              <StyledTableCell align="right" sx={TextHover}>{project.campus}</StyledTableCell>
              <StyledTableCell align="right" sx={TextHover}>{project.faculty}</StyledTableCell>
